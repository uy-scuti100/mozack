...
export async function formatProduct(product, fields) {
	const formattedProduct = formatProductData(product, fields);
	const productMediaItems = extractNestedArrayData(product, "media.items", {
		title: "title",
		imageUrl: "image.url",
	});
	formattedProduct.productMediaItems = productMediaItems;
	return formattedProduct;
}
